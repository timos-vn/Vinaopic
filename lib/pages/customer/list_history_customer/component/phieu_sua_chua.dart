...
class PhieuSuaChuaPage extends StatefulWidget {
  final String maKH;

  const PhieuSuaChuaPage({Key? key,required this.maKH}) : super(key: key);
  @override
  _PhieuSuaChuaPageState createState() => _PhieuSuaChuaPageState();
}

class _PhieuSuaChuaPageState extends State<PhieuSuaChuaPage> {
  ...
  void initState() {
    ...
    _bloc.add(GetListHistoryCustomerEvent(widget.maKH,3));
    _scrollController = ScrollController();
    _scrollController.addListener(() {
      final maxScroll = _scrollController.position.maxScrollExtent;
      final currentScroll = _scrollController.position.pixels;
      if (maxScroll - currentScroll <= _scrollThreshold && !_hasReachedMax && _bloc.isScroll == true) {
        _bloc.add(GetListHistoryCustomerEvent(widget.maKH,3,isLoadMore: true,));
      }
    });
  }
