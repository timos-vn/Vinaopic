...
class PhieuKhamPage extends StatefulWidget {
  ...
  const PhieuKhamPage({Key? key,required this.maKH}) : super(key: key);
  @override
  _PhieuKhamPageState createState() => _PhieuKhamPageState();
}
...
class _PhieuKhamPageState extends State<PhieuKhamPage> {
  ...
  void initState() {
    ...
    _bloc.add(GetListHistoryCustomerEvent(widget.maKH,2));
    _scrollController = ScrollController();
    _scrollController.addListener(() {
      final maxScroll = _scrollController.position.maxScrollExtent;
      final currentScroll = _scrollController.position.pixels;
      if (maxScroll - currentScroll <= _scrollThreshold && !_hasReachedMax && _bloc.isScroll == true) {
        _bloc.add(GetListHistoryCustomerEvent(widget.maKH,2,isLoadMore: true,));
      }
    });
  }
