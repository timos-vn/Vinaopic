...
import 'package:vinaoptic/widget/text_field_widget2.dart';
          ...
          child: BlocListener<SignUpBloc, SignUpState>(
            listener: (context, state) {
              if (state is SignUpFailure){
                Utils.showCustomToast(context, Icons.warning_amber, state.error);}
              if (state is SignUpSuccess){
                Utils.showCustomToast(context, Icons.check, 'Đăng ký tài khoản thành công');
      ...
      onPressed: ()async
          {
            if( _phoneController.text.length > 4){
              ...
              Utils.showCustomToast(context, Icons.warning, 'Vui lòng nhập tên đăng nhập của bạn');
  ...
  Padding buildInputPhoneNumber(BuildContext context) {
    return
      Padding(
        ...
        onChanged: (text) => _signUpBloc.add(ValidateUserName(text.toString())),
      ),
    );
  }
    ...
    return
      Padding(
        ...
        errorText: _errorPassWord,
         isPassword: true,
        onChanged: (text) => _signUpBloc.add(ValidatePassWord(text.toString())),
      ),
    );
  }
