...
import 'package:vinaoptic/pages/appointment_schedule/appointment_schedule_state.dart';
...
class ChoTuVanPage extends StatefulWidget {
  @override
  _ChoTuVanPageState createState() => _ChoTuVanPageState();
}

class _ChoTuVanPageState extends State<ChoTuVanPage> {
  ...
  void initState() {
    ...
    _bloc.add(GetList(Utils.parseDateToString(DateTime.now(), Const.DATE_SV_FORMAT_2),Utils.parseDateToString(DateTime.now(), Const.DATE_SV_FORMAT_2), status: '4'));
    ...
    _scrollController.addListener(() {
      ...
      if (maxScroll - currentScroll <= _scrollThreshold && !_hasReachedMax && _bloc.isScroll == true) {
        _bloc.add(GetList(Utils.parseDateToString(DateTime.now(), Const.DATE_SV_FORMAT_2),Utils.parseDateToString(DateTime.now(), Const.DATE_SV_FORMAT_2),isLoadMore: true, status: '4'));
      }
    });
  }
        ...
        body: BlocListener<AppointmentScheduleBloc,AppointmentScheduleState>(
            ...
            child: BlocBuilder<AppointmentScheduleBloc,AppointmentScheduleState>(
              ...
              builder: (BuildContext context, AppointmentScheduleState state){
                List<AppointmentScheduleData> _list = _bloc.list;
